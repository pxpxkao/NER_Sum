...
class data_utils():
    def __init__(self, args):
        ...
        label_idx_path = args.labelmap
        if self.add_ner:
            self.label2id = read_json(label_idx_path)
            print('reading label idx from %s'%label_idx_path)
    ...
    def data_yielder(self, src_file, tgt_file, ner_file, num_epoch = 100):
        ...
        if self.train:
            ...
            for epo in range(num_epoch):
                ...
                files = (open(src_file), open(tgt_file), open(ner_file)) if self.add_ner else (open(src_file),open(tgt_file))
            ...
            for epo in range(1):
                ...
                files = (open(src_file),open(ner_file)) if self.add_ner else (open(src_file))
                for line in zip(*files):
                    if self.add_ner:
                        line1, line2 = line 
                    else:
