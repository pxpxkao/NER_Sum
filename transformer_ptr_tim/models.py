...
class EncoderDecoder(nn.Module):
    ...
    other models.
    """
    def __init__(self, encoder, decoder, src_embed, tgt_embed, linear = None):
    ...
    def forward_ner(self, src, tgt, src_mask, tgt_mask, src_extended, oov_nums, ner):
        "Take in and process masked src and target sequences."
        # return self.decode(self.encode(src, src_mask), src_mask,
        #                     tgt, tgt_mask, src)
        return self.decode(self.encode_ner(src, src_mask, ner), src_mask,
                            tgt, tgt_mask, src_extended, oov_nums)
    
    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)

    def encode_ner(self, src, src_mask, ner):
        src_embed = torch.cat((self.src_embed(src), ner), dim=-1)
        return self.encoder(self.linear(src_embed), src_mask)
