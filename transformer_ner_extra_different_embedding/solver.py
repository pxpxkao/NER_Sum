...
class Solver():
    ...
    def train(self):
        ...
        for step in range(1000000):
            ...
            out = self.model.forward(batch['src'], batch['tgt'], batch['src_mask'], batch['tgt_mask'], batch['ner'], batch['src_extended'], len(batch['oov_list']))
            ...
            if step % 500 == 1:
                ...
                pp =  self.model.greedy_decode(batch['src_extended'].long()[:1], batch['src_mask'][:1], 100, self.data_utils.bos, batch['ner'][:1], len(batch['oov_list']), self.data_utils.vocab_size)
    ...
    def test(self):
        ...
        for batch in data_yielder:
            ...
            if self.args.beam_size == 1:
                out = self.model.greedy_decode(batch['src'].long(), batch['src_mask'], max_len, self.data_utils.bos, batch['ner'], len(batch['oov_list']), self.data_utils.vocab_size)
            else:
