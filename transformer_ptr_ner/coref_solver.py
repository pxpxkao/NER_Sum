...
class CorefSolver():
    ...
    def make_model(self, src_vocab, tgt_vocab, N=6, dropout=0.1,
            ...
            if entity_encoder_type == 'transformer':
                ...
                print('transformer')
                entity_encoder = Seq_Entity_Encoder(embed, Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), 2))
                ...
                entity_encoder = RNNEncoder(embed, 'GRU', d_model, d_model, num_layers=1, dropout=0.1, bidirectional=True)
                print('gru')
            elif entity_encoder_type == 'lstm':
                entity_encoder = RNNEncoder(embed, 'LSTM', d_model, d_model, num_layers=1, dropout=0.1, bidirectional=True)
                print('lstm')
    ...
    def train(self):
        ...
        for step in range(start_step, self.args.total_steps):
            ...
            if self.args.entity_encoder_type == 'gru' or self.args.entity_encoder_type == 'lstm':
                ner_feat = self.model.entity_encoder(batch['ner'].transpose(0, 1), batch['cluster_len'])[1]
            ...
            ner, ner_mask = self.data_utils.pad_ner_feature(ner_feat.squeeze(), batch['num_clusters'], batch['src'].size(0))
            # print('ner', ner.size())
            # print('ner_mask', ner_mask.size())
            ...
            if step % self.args.print_every_steps == 1:
                ...
                if ner_mask != None and not self.args.ner_at_embedding:
                    pp = self.model.greedy_decode(batch['src_extended'].long()[:1], ner[:1], batch['src_mask'][:1], 100, self.data_utils.bos, len(batch['oov_list']), self.data_utils.vocab_size, True, ner_mask[:1])
                else:
                    pp = self.model.greedy_decode(batch['src_extended'].long()[:1], ner[:1], batch['src_mask'][:1], 100, self.data_utils.bos, len(batch['oov_list']), self.data_utils.vocab_size, True)
            ...
            if step % self.args.valid_every_steps == 2:
                ...
                for batch in val_yielder:
                    with torch.no_grad():
                        ...
                        if self.args.ner_at_embedding:
                            pred = self.model.greedy_decode(batch['src_extended'].long(), ner, batch['src_mask'], self.args.max_len, self.data_utils.bos, len(batch['oov_list']), self.data_utils.vocab_size)
                        else:
                            pred = self.model.greedy_decode(batch['src_extended'].long(), ner, batch['src_mask'], self.args.max_len, self.data_utils.bos, len(batch['oov_list']), self.data_utils.vocab_size, ner_mask=ner_mask)
                ...
                scores = cal_rouge_score(self.w_valid_file, self.args.valid_ref_file)
