...
class data_utils():
    ...
    def data_yielder(self, src_file, tgt_file, num_epoch = 100):
        print('Reading source file from %s ...'%src_file)
        print('Reading target file from %s ...'%tgt_file)
        print(self.batch_size)
        src_length = 400
        tgt_length = 100
        if self.train:
            batch = {'src':[],'tgt':[],'src_mask':[],'tgt_mask':[],'y':[], 'src_extended':[], 'oov_list':[], 'ner':[], 'ner_text':[]}
            oov_list = []
