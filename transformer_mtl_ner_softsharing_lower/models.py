...
class EncoderDecoder(nn.Module):
    ...
    other models.
    """
    def __init__(self, encoder_sum, encoder_ner, encoder_sum_upper, decoder, generator, src_embed, tgt_embed):
        super(EncoderDecoder, self).__init__()
        self.encoder_sum = encoder_sum
        self.encoder_ner = encoder_ner
        self.encoder_sum_upper = encoder_sum_upper
    ...
    def forward(self, src, src_mask, tgt=None, tgt_mask=None, sum = True):
        ...
        if sum:
            out = self.decode(self.encode_sum(src, src_mask), src_mask,
                            tgt, tgt_mask, src)
        else:
            out = self.generator(self.encode_ner(src, src_mask))
        return out
    
    def encode_sum(self, src, src_mask):
        out = self.encoder_sum(self.src_embed(src), src_mask)
        return self.encoder_sum_upper(out, src_mask)
    ...
    def mtl_loss(self):
        sum_params = []
        loss = []
        for name, param in self.encoder_sum.named_parameters():
            if param.requires_grad:
                # print(name, param.data)
                sum_params.append((name, param.data.clone()))
        i = 0
        for name, param in self.encoder_ner.named_parameters():
            if param.requires_grad:
                # pr-int(name, param.data)
                if name != sum_params[i][0]:
                    print('fuck', name, sum_params[i][0])
                else:
                    loss.append(torch.norm(param - sum_params[i][1])/torch.numel(param))
                i+=1
    ...
    def greedy_decode(self, src, src_mask, max_len, start_symbol):
        ...
        memory = self.encode_sum(src, src_mask)
