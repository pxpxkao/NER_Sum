...
def make_dict(max_num, dict_path, train_ner_path, train_sum_path, train_tgt_path):
    word_count = dict()
    word2id = dict()
    line_count = 0
    ...
    for line in tqdm(open(train_tgt_path)):
        line_count += 1.0
        for word in line.split():
            word_count[word] = word_count.get(word,0) + 1
...
class data_utils():
    def __init__(self, args):
        ...
        labeldic_path = '../data/label_dict.json'
        self.train_ner_path = args.train_ner
        self.train_sum_path = args.train_src
        self.train_tgt_path = args.train_tgt
        self.train_ner_tgt_path = args.train_ner_tgt
        if os.path.exists(dict_path):
            self.word2id = read_json(dict_path)
        else:
            self.word2id = make_dict(30000, dict_path, self.train_ner_path, self.train_sum_path, self.train_tgt_path)
        ...
        if os.path.exists(labeldic_path):
            ...
            self.label2id = make_labeldic(labeldic_path, '../data/label-map.index')
    ...
    def data_yielder_ner(self, num_epoch = 1, data_dir = '../data/conll_data/'):
            ...
            for line1, line2 in zip(open(os.path.join(data_dir, 'train.txt')), open(os.path.join(data_dir, 'label.txt'))):
                ...
                if vec1 is not None and vec2 is not None:
                    ...
                    if len(batch['src']) == self.batch_size:
                        ...
                        batch = {'src':[], 'src_mask':[],'y':[]}
            end_time = time.time()
            print('finish ner epo %d, time %f' % (epo,end_time-start_time))

    def data_yielder_sum(self, num_epoch = 20, data_dir = '../data/cnn/'):
        batch = {'src':[],'tgt':[],'src_mask':[],'tgt_mask':[],'y':[]}
        for epo in range(num_epoch):
            start_time = time.time()
            print("start epo %d" % (epo))
            # index = 0
            for line1,line2 in zip(open(os.path.join(data_dir, 'train.txt.src')),open(os.path.join(data_dir, 'train.txt.tgt.tagged'))):
