...
class Solver():
    ...
    def train(self):
        ...
        for step in range(1000000):
            ...
            # print(lamda*loss_mtl)
            # print(loss)
            loss.backward()
            optim.step()
            
            total_loss.append(loss.detach().cpu().numpy())
            mtl_losses.append(1000 * loss_mtl.detach().cpu().numpy())
            
            if step % 500 == 1:
                elapsed = time.time() - start
                print("Epoch Step: %d Loss: %6.6f MTL Loss: %6.6f Time: %f" %
                        (step, np.mean(total_loss), np.mean(mtl_losses), elapsed))
                print('Src:',self.data_utils.id2sent(gg[0]))
                print('Tgt:',self.data_utils.id2sent(tt[0]))
                print('Pred:',self.data_utils.id2sent(pred_sum[0]))


                pp =  self.model.greedy_decode(batch_sum['src'].long()[:1], batch_sum['src_mask'][:1], 100, self.data_utils.bos)
